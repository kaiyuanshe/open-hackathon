...
class Provider:
    def __init__(self, provider_name, provider_cfg):
        if provider_name not in _provider_classes:
            raise ProviderError("{} provider not found", provider_name)
        self.provider_name = provider_name
        self.p_class = _provider_classes[provider_name]
    ...
    def start_expr(self, experiment, template_content):
        # TODO
        pass

    def pause_expr(self, experiment, template_content):
        # TODO
        pass

    def delete_expr(self, experiment, template_content):
        LOG.info("deleting experiment {}".format(experiment.id))

        try:
            self.p.delete_instance(self._get_ve_config(experiment, template_content))
        except Exception as e:
    ...
    def _init_ve_config(self, experiment, template_content):
        if self.provider_name == VE_PROVIDER.K8S:
        ...
        raise ProviderError("not found {} instance config".format(self.provider_name))


def registry_provider(provider_name, provider_class):
