...
from app.enum import ExprStatus, EmailStatus
from app.functions import safe_get_config
from app.hack import hack_manager
...
class UserManager(object):
    ...
    def __generate_api_token(self, user):
        ...
        user_token = AdminToken(str(uuid.uuid1()), user, token_expire_date, token_issue_date)
    ...
    def db_login(self, openid, **kwargs):
        ...
        admin = self.db.find_first_object(Admin, openid=openid)
        if admin is not None:
            ...
            for n in email_info:
                ...
                verified = n['verified']
                useremail = AdminEmail(name=kwargs['name'], email=email, primary_email=primary_email,
                                      verified=verified, admin=admin)
